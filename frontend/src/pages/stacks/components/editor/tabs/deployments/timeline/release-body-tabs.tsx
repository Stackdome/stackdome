...
export function ReleaseBodyTabs({ children, diff, hasPrev, prevSeq, loading = false }: ReleaseBodyTabsProps) {
  ...
  const tabClass = (id: TabId) =>
    cn(
      "-mb-px flex items-center gap-1.5 border-b-2 pb-2 text-meta font-medium",
    ...
    <div className="mt-4">
      <div className="flex items-center gap-5 border-b border-border">
        ...
        <button onClick={() => setTab("changes")} className={tabClass("changes")}>
          Changes
          {changeCount > 0 && <span className="font-mono text-label text-fg-muted">{changeCount}</span>}
        </button>
        {tab === "changes" && hasPrev && (
          <span className="ml-auto pb-2 font-mono text-label text-fg-muted">vs #{prevSeq ?? "previous"}</span>
        )}
      </div>
