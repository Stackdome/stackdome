...
| **Real app in a browser, no backend** | `pnpm --prefix frontend dev:mock` (port 5273) |
| Same, but a brand-new org — first-run/empty states | `pnpm --prefix frontend dev:mock:empty` (port 5274) |
...
- **`dev:mock` runs the real app — real router, real shell, real pages — against a mocked network** (`src/preview/`). No Go server, Postgres or Kind cluster. Storybook judges components; this judges the product with navigation that actually works. Handlers reuse `.storybook/fixtures.ts`, which is typed off the generated OpenAPI schemas, so a wrong response fails to compile.
- **Empty and first-run states need `dev:mock:empty` (port 5274).** The normal preview serves a populated dataset, so no amount of clicking reaches the state a new user actually lands on. The scenario is set by `VITE_PREVIEW_SCENARIO` **before** the service worker boots — it cannot be a URL param — and the two run on separate ports so both can be open at once. `/stacks` and the **git integrations** are blanked — the second is what makes the New stack page's "no provider connected" state reachable; a new org still has a project and a cluster.
- **Two untracked Playwright scratch harnesses** (gitignored): `frontend/.shot.mjs` renders stories/routes to PNG, `frontend/.measure.mjs` reads computed geometry. Design work is verified by **measuring**, not eyeballing — and assertions must cover the *gaps between* elements, not only their positions.
- **Anything visual: read `DESIGN-PRODUCT.md` first.** It holds the product's design rules — surfaces, radius ladder, type scale, colour tiers, control heights, button material. If a rule and the code disagree, the code is wrong. Two rules bind hardest: **update the existing primitive rather than creating a new component** (ask first), and **never hand-roll a copy of a component inside a story**. Reasoning and history live in `docs/design/redesign-log.md`.
